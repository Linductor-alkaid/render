...
- [ ] **2.1.2** 实现各形状的 AABB 计算
- [ ] **2.1.3** 实现惯性张量计算（用于刚体初始化）
...
- [ ] **2.2.2** 实现空间哈希粗检测 `SpatialHashBroadPhase`
  - 设计哈希函数（64位哈希）
  - 实现格子大小自适应
  - 处理跨格子物体
  - 去重检测对

- [ ] **2.2.3** 实现八叉树粗检测 `OctreeBroadPhase`（可选，用于静态场景）
  - 递归细分
  - 动态物体更新
  - 查询优化

- [ ] **2.2.4** 添加粗检测配置选项到 `PhysicsConfig`
  ```cpp
...
- [ ] **3.3.1** 实现 `PhysicsUpdateSystem` 固定时间步长
  ```cpp
