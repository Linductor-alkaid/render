...
**⚡ 性能优化（v0.12.0）**:
- **两阶段锁定**: 快速读取纹理ID（持锁，微秒级），然后执行OpenGL调用（无锁）
- **减少锁竞争**: OpenGL调用不再阻塞其他线程访问Texture对象
- **提高并发性**: 多个纹理可以同时绑定到不同单元而不互相阻塞
- **避免死锁**: 消除了与Material::Bind()的潜在锁顺序问题

**线程安全**:
- ✅ 可以从任意线程调用（OpenGL调用会在主线程执行）
- ✅ 无锁竞争：读取纹理ID后立即释放锁
- ✅ 数据安全：锁内读取保证纹理ID的有效性
