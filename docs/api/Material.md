...
1. 激活着色器
2. 设置所有材质属性到 uniform
3. 绑定所有纹理（并自动设置纹理存在标志，如 `hasDiffuseMap`）
4. 设置所有自定义参数
5. 应用渲染状态（如果提供了 renderState）

**⚡ 性能优化（v0.12.0）**:
- **两阶段锁定**: 先在锁内快速拷贝数据（微秒级），然后在锁外执行OpenGL调用（毫秒级）
- **避免嵌套锁**: 应用渲染状态时不再调用持锁方法，消除潜在死锁
- **减少锁竞争**: OpenGL调用不再阻塞其他线程访问Material对象
- **性能提升**: 锁持有时间减少约95%

**线程安全保证**:
- ✅ 数据一致性：锁内复制的数据保证是同一时刻的快照
- ✅ 无死锁风险：消除了嵌套锁和锁顺序问题
- ✅ 高并发性：OpenGL调用期间其他线程可访问Material
- ✅ 纹理绑定优化：Texture::Bind()也使用两阶段锁定
