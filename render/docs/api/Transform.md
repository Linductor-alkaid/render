...
修改变换时只标记脏标志，延迟到实际需要时才计算：

```cpp
transform.SetPosition(pos);      // 仅标记 dirty
transform.SetRotation(rot);      // 仅标记 dirty
transform.SetScale(scale);       // 仅标记 dirty
Matrix4 mat = transform.GetLocalMatrix();  // 现在才计算
```
